...
## Full WebGPU Validation Progress — 2026-09-09 UTC

- Integrated engine template build succeeds, including native Tint precompilation and Mono/WASM link, in engine-23573-15805.log. Output: artifacts/webgpu-engine-f329e39c/output/20260909T195456Z. WSL compiler setup installed 26 dependency packages, upgraded libc6, removed no packages; WSL closed after completion. This is engine-build success only; actual game/browser validation follows.
- Serial Dawn warmup succeeds with private caches. Native WebGPU compilation reaches the generated WGSL table, then fails because the candidate additionally builds a native Tint converter and this WSL environment has no c++ command. Added an explicit no-package-removal host-compiler setup and a fast prerequisite check; no published engine or game files changed.
- Resolved the sole support-list insertion conflict with a checked union; all 1,296 engine files now prepare and AST validation confirms both mono and webgpu. Managed assemblies, SDK packages and threadless runtime pack build; first native compile stops on a RemoteDisconnected while acquiring Emscripten Dawn, not on a C# or game error. Separate Emscripten/NuGet caches and serial port warmup avoid parallel first-download contention; lock files are not deleted. One out-of-order multi-hunk patch was atomically rejected before the corrected patch applied.
- Acquired and Git-blob-verified 1,344 fixed source/base files (20,995,745 bytes), with one transient fetch retry. Isolated source backport merged 1,295 files; platform/web/detect.py has one explicit conflict. Original Mono sources remain unchanged and Ubuntu was terminated in finally. Preparation correctly failed rather than claiming build success; conflict and log preserved in artifacts/webgpu-engine-f329e39c.
- Resolved real upstream identities and full trees: 1,296 engine-scope changes, 48 modified existing files, zero file removals. Added pinned/raw-blob-hash source acquisition and a separate Mono-cache copy plus fail-closed three-way merge. One manually mistyped tree SHA returned 404; subsequent queries use the resolved commit value rather than retyping it.
- Started from a clean worktree and inspected existing platform/rebirth/threadless tools. No new release, server changes or file deletion is authorized by this validation request.

---
## Actual-Game WebGPU Validation Result — 2026-09-09 UTC

- Staged whitespace audit caught redundant EOF blank lines in eight new tool files; removed only those blank lines and reran the check before committing. No file was deleted.
- Final tools checks: 37 Node tests and revised WebGL2 three-load Reimu benchmark pass again; 34 changed texts are UTF-8 without BOM/NUL, 16 JS files parse, four PowerShell 7 scripts and the Python preparation script parse. Existing-output refusal preserves the old report hash. No task-owned verifier or WSL distribution remains running. These records and the NO-GO report are included in the local commit; no production acceptance is implied.
- Built the isolated Mono+WebGPU+WebGL2 engine and actual unchanged C# game. Main verification: artifacts/webgpu-validation/2026-09-09T20-26-51-285Z, 47 groups, 29 script passes / 18 failures. Node 37/37, core 40/40, six deterministic journeys, localization 860/860 and native standalone 28 checks rerun successfully.
- The browser compositor exposes missing bulk sprites and ground in all six matching static fixtures. Twelve captures are valid, but all six equivalence gates fail. Numeric WebGPU FPS is therefore diagnostic only, not equal-work performance acceptance. Evidence: artifacts/webgpu-validation/2026-09-09T21-24-20-589Z/visual-comparison.
- Other failures: synchronous image readback, volume/language persistence after reload, three browser-lifecycle stalls and four actual-game fallback/loss cases. Kept reports and safely ended only the three verified owned process trees. Added bounded diagnostics; no engine/gameplay fix was claimed. Revised WebGL2 five-scenario UI regression passes again at artifacts/webgpu-validation/2026-09-09T21-28-35-445Z.
- Matching 60-second samples: WebGL2 Reimu 59.45 mean/51 minimum FPS; Marisa 32.79/5. WebGPU timings cannot support a speed claim because it omits visual work. Marisa 40/180 fixtures stop in Choosing and are invalid continuous-load measurements. No physical mobile acceptance.
- All 179 shared staged inputs including 81 C# files match. Protected artifacts remain identical. Full report: docs/webgpu_full_validation.md. No release/version/deployment/push/deletion.
...
## Growth Release Implementation Checkpoint — 2026-09-13

- Source 970547a builds a 195251800-byte self-contained Windows candidate with 36 checks and optimized compatible Web build 20260913-051207-734 (21637092 core gzip bytes). New real-browser stance checks pass all eight bilingual desktop/touch combinations; Reimu Boss passes. Marisa Boss fixture stops at exactly the initial 1.5-second emission boundary, so its frozen snapshot has zero hostile stars; the positive-star assertion correctly fails. Prime its first emission explicitly in the diagnostic fixture, retain this candidate and rebuild both platforms from the corrected source. Do not weaken the hostile-star check or publish the failed candidate.
- First wrapper export attempt failed because a relative Tee-Object log path followed the staged Push-Location. Absolute log paths fix it; failed stage retained. Import logs contain original audio-tag UTF-8 warnings; C# builds and actual runtime checks are clean. No source art or audio was changed to hide metadata warnings.

- Final native candidate: core 68/68; localization 1108/1108; font 1294 characters; both 60-minute assisted soaks pass with 44/47 growth investments and 5/7 extended wins. Corrected unassisted routes: Rapid 2/3, Charged 2/3, Young 1/3, Mature 2/3; completed runs 1097.2–1141.8 seconds and 26 pre-Boss choices. Failed pilot seeds are retained, not described as universal balance success.
- Native batch comparison passes both heroes (81 base checks plus 24 color checks each). Seven new/current captures reviewed; fixed pale build-detail text by using the existing pixel theme ink, separated the summary row from training, and reframed the diagnostic Boss so sprites are visible. Normal gameplay camera and art remain unchanged. Added contrast regression.
- Read-only lookup of a guessed core WebCheckState path failed; located the actual presentation file. No files removed. Next commit/export/verification stage still must precede publication.

- Core 67/67, native build, bilingual UI/settings/journal/stance-confirmation and legacy/continuation records pass. Hour-long assisted soaks include repeated character encounters and bounded storage. Unassisted tests exposed long-run attrition; added finite collectible recovery windows, tuned XP budgeting, and tested all four stances. Final run must follow the corrected extra-pierce convention (zero means no extra target), not the intermediate off-by-one candidate.
- Preparing alpha-0.1.9 metadata and preserved-history changelog. No candidate has been exported or deployed yet; next steps are final source checks, immutable same-commit builds and browser/public validation.

- Core implements canonical character profiles, opposite-character Boss attacks, independent enemy stars/beam/recovery, safe delayed owner cleanup, single-mainline stances, repeatable growth, 18-minute Boss scheduling and voluntary five-minute continuation cycles. Presentation adds confirmation, character sprites, dangerous effect cues and separate continuation records; unfinished content and balance are not accepted yet.
- First core run compiles and passes 52/60. Resolve failed offer/timeline/queued-XP expectations, disable actual Boss abilities in inert DPS fixtures, reassess heavy-star sampling, and translate all new strings before acceptance. Add dedicated growth/Boss/continuation tests rather than weakening old performance or matched-DPS limits.
- Restored context after handoff. Root AGENTS.md read reports absent (the supplied user instructions remain applicable); existing large plan/history was truncated by CMD output, now reading bounded UTF-8 sections. No files deleted.
