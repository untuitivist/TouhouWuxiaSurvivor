...
async def generate(workflow, run_name):
    ...
    command = [sys.executable, "-u", str(COMFY_ROOT / "main.py"), "--cpu", "--force-fp32", "--disable-all-custom-nodes", "--disable-api-nodes", "--disable-auto-launch", "--cache-none", "--listen", "127.0.0.1", "--port", "8189", "--base-directory", str(workspace), "--models-directory", str(MODEL_DIRECTORY.parent), "--output-directory", str(OUTPUT), "--user-directory", str(workspace / "user"), "--database-url", "sqlite:///:memory:"]
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60)) as session:
            ...
            async with session.ws_connect(BASE_URL + "/ws?clientId=" + client_id, heartbeat=30, max_msg_size=16 * 1024 * 1024) as websocket:
                ...
                saved = []
                for image in images:
                    filename = (OUTPUT / image["subfolder"] / image["filename"]).resolve()
                    if not filename.is_relative_to(OUTPUT.resolve()) or not filename.is_file():
                        raise RuntimeError("Invalid output image path")
                    saved.append({"file": filename.relative_to(OUTPUT).as_posix(), "sha256": hashlib.sha256(filename.read_bytes()).hexdigest()})
                ...
                with (OUTPUT / f"{run_name}.json").open("x", encoding="utf-8") as stream:
...
if __name__ == "__main__":
    ...
    if (OUTPUT / f"{arguments.run_name}.json").exists():
    ...
    asyncio.run(generate(workflow, arguments.run_name))
