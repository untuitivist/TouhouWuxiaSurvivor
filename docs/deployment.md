...
## alpha-0.1.11 双端发布完成（2026-09-13 UTC／北京时间 2026-09-14）

- 冻结源码 `e4306bf765814fcd7accd5263f7c1343cef571a4` 已推送并由服务器 git pull --ff-only 同步；维护仓库、Windows 与 Web stage 的 101 个 C# 文件逐字节一致，Windows 203 个输入哈希复核通过。后续交付文档提交不改变产物来源。
- Windows：release/TouhouWuxiaSurvivor_alpha-0.1.11.exe，195,276,448 字节，文件版本 0.1.11.0；SHA-256 `4D9A584EC070C9519F7A556A2AE992DD72F8E54D36FD6ACB5B84812ACE1F9C8D`。内嵌 .NET 8.0.6 与 PCK，仅 EXE 的隔离目录中 36 项检查通过；没有上传 GitHub Release 二进制资产。
- Web：干净源码构建 20260914-025152-678，启用 `alpha-0.1.11-e4306bf-20260913T190912Z`。继续同一 C# 项目、优化 Mono／WebGL2 单线程，无跨源隔离与无 SharedArrayBuffer 入口均通过；未启用 WebGPU。
- 本版恢复第 240 秒首领与约五分钟标准局，经验 10+7×等级；单次成长体现数量、范围、宽度或明显威力变化。自由三选一、相容组合、六术式独立精进与可选续战保留，无定式、主副或路线名额。
- 首领改用专属相位／环幕／快照魔炮，不持有玩家成长或引力发射状态。环幕 1 秒预警并保留两条通道；魔炮 1.5 秒预警、发射 1.6 秒，预警开始固定世界原点与射向。阶段切换清自身旧弹并留喘息；人物人设、原作纹理与批量绘制保持。
- 正式源码重验：核心 74/74、翻译 1128/1128、JavaScript 22/22、Python 20/20；独立 EXE 36 项通过。正式 Web 的 20 项成长／首领用例、20 次原始加载、raw/gzip × 有/无隔离、双角色普通与注入颜色、加载异常、小地图、23 条图鉴四布局、立绘与标题全部通过。
- 原性能门槛的两角色 × 两语言 × 三档共 12 项全部通过。最高档 320 敌人、约 1590 弹幕、约 390 拾取物，桌面 Edge 152／RTX 4070 Ti 平均与最低采样均 60 FPS；灵梦模拟 P95 为 1.9／2.0 ms，魔理沙 6.0／5.8 ms。未降低负载；不代表手机／平板真机 60 FPS。
- 平衡证据沿用逐字相同游戏逻辑的 c94e17f 发布前完整测量：18/18 无辅助诊断通关，255.7–323.6 秒、首领前 17–21 次成长，同投入 DPS 比值约 0.971–1.043。版本提升后重新跑核心与全部发行物门禁，没有冒称重跑了全部平衡样本；恢复辅助一小时续战只证明稳定性。
- 公网明确禁用代理的桌面、触屏模拟、无隔离触屏三个入口均通过；本机当次首载 10.120／10.113／9.861 秒，设置刷新保存、多指操作与竖屏暂停正常，无运行错误或失败请求。已查看线上 alpha-0.1.11 标题和无隔离触屏实战截图；不是普遍线路速度保证。
- 核心计量下载 21,652,535 字节：PCK gzip 9,793,063，WASM gzip 11,859,472；不含页面／脚本／协议。活动目录全部 gzip 21,752,092 字节；独立直连 HTTPS 入口 SHA-256 `6f07e6d44b475705d84894af767fcdebad60d1c956a5315a4efd18251ced75a5` 与固定 SSH 主机 active.json 一致，服务器源码与配套 EXE 哈希一致。既有根站点、/tusharedata/ 和拒绝路由状态保留。
- 回滚目录 `/srv/touhou-survivor/backups/alpha-0.1.11-e4306bf-20260913T190912Z/`。完整日志快照与回执为 release/CHANGELOG_alpha-0.1.11.md、release/RECEIPT_alpha-0.1.11.json；总回执 artifacts/release-receipts/alpha-0.1.11.json，证据索引 artifacts/short-run-release-20260914/。41 份旧发行文件及所有旧更新条目哈希不变，没有删除文件。
- 初轮临时验证包装脚本遗漏 NODE_PATH，Playwright 未解析，已修正为既有模块目录后全部重跑；失败日志保留，没有更改产品代码或豁免门槛。推送使用既有本机代理，公网验收明确直连。新提议的固定技能弹射对战只作设计讨论，没有创建分支或新模式；旧标签页需刷新确认左下角 alpha-0.1.11。

## alpha-0.1.10 双端发布完成（2026-09-13）

- 冻结源码 `b17b5c53ea20fe1968281425827a3d039563b504`，已推送并由服务器 git pull --ff-only 同步；维护仓库与 Windows／Web stage 的 99 个 C# 文件逐字节一致。后续交付文档提交不改变游戏产物来源。
- Windows：release/TouhouWuxiaSurvivor_alpha-0.1.10.exe，195,261,576 字节，文件版本 0.1.10.0；SHA-256 `EAC4C154A759BFD8D88CC2253F22706A6176CF43431789AD587F61917DF4D958`。内嵌 .NET 8.0.6 与 PCK，仅一个 EXE 的隔离目录内完成 36 项独立检查；没有上传 GitHub Release 二进制资产。
- Web 构建 20260913-171454-752，启用 `alpha-0.1.10-b17b5c5-20260913T093042Z`。仍为同一 C# 项目、优化 Mono／WebGL2 单线程兼容工具链；无跨源隔离、无 SharedArrayBuffer 也通过完整流程。未启用 WebGPU，没有更改视角或美术路线。
- 本版撤除整个定式框架、第四次强制选边、兄弟路线排除和确认状态。六术式独立持续精进，相容行为共同生效；可均衡、可集中、后续可转投。三选一单次操作保留，查看构筑不消耗或刷新选项；有限经验形成机会成本，不新增主副路线或学习名额。
- 阴阳玉、魔炮和符卡补齐敌星消除，普通弹幕与星体共用原预算，敌我过滤和延迟安全回收保持。旧存档定式字段只读兼容，新记录不再选择定式。18 分钟角色首领、自机排除、有限回收窗口和自愿续战保留。
- 验收：核心 72/72、翻译 1104/1104、JavaScript 22/22、Python 20/20、字体 1299 字符；原生界面／旧档案／双语／批次通过。Web 新成长 12 项、20 次原始加载、raw/gzip × 有/无隔离四组各五项流程、双角色普通/注入颜色、六类加载异常、小地图、23 条图鉴四布局、选人立绘和标题全部通过。
- 性能：本机桌面 Edge／RTX 4070 Ti，无隔离、无共享内存，双角色双语三档均通过原门槛。最高档 320 敌人、约 1591 弹幕的平均／最低均 60 FPS；灵梦逻辑 P95 为 2.0／2.2 ms，魔理沙 5.8／5.7 ms。不是物理手机／平板性能承诺，未减少压力预算。
- 平衡：同投入单体 DPS 保持原 0.8–1.2 比值门槛。三种子无辅助诊断中，灵梦均衡／基础集中／辅助集中分别 3/3、2/3、1/3 胜局，魔理沙各 3/3，首领前 25–26 次成长；偏好只是测试输入，不是预设流派。两角色各一小时恢复辅助续战只证明稳定性，不当成人类胜率。
- 公网明确禁用代理的桌面、触屏模拟、无隔离触屏三个入口均通过；首载分别 14.790、11.563、10.365 秒，设置保存刷新、多指移动／慢移／闪身和竖屏暂停通过，无运行错误或失败请求。已查看公网版本 alpha-0.1.10 标题和无隔离触屏实战截图；仅代表本机当次线路。
- 核心计量下载 21,646,930 字节：PCK gzip 9,787,458，WASM gzip 11,859,472；不含页面／脚本／协议。活动目录全部 gzip 共 21,746,488 字节。入口 SHA-256 `328dd502f7766662c85ea05a6ac3ba32e2b0c0133eb102ae1d69b2197bc6088d` 与固定 SSH 主机的 active.json、服务器 Git 和 Windows 哈希交叉核对一致；既有站点路由、首载减重与 BBR 配置保留。
- 失败证据保留：首候选 5580e04 的 Web 灵梦长局未过，未部署。诊断驾驶员补重叠敌星的输入预测后，原种子、原胜利断言与所有门槛通过；没有加血、改伤害或降低首领生命。旧候选 EXE／报告归档到 artifacts/freebuild-release-20260913/candidate-5580e04，原 Web 失败和非发布证明构建均保留。Git 直连重置／超时后仅对推送使用已有本地代理，未修改系统设置；临时发布自检的混合路径分隔符已规范化。
- 全部旧更新日志与已发布 EXE 保留，alpha-0.1.9 EXE 哈希复核未变。回滚目录 /srv/touhou-survivor/backups/alpha-0.1.10-b17b5c5-20260913T093042Z。回执为 release/RECEIPT_alpha-0.1.10.json 与 artifacts/release-receipts/alpha-0.1.10.json；完整版本日志 release/CHANGELOG_alpha-0.1.10.md，证据索引 artifacts/freebuild-release-20260913/。
- 已知旧文案：标题页仍写“约五分钟一局”，实际标准首领为第 18 分钟。记录为后续文案修正，不在冻结产物后静默修改；本版不宣称所有历史问题已清零。已打开旧页面需刷新或重新打开。
