...
## alpha-0.1.9 双端发布完成（2026-09-12 UTC／北京时间 2026-09-13）

- 冻结源码 `e65d63dbc682f872f2a7288288ea3cd8fec19297`，已推送并由服务器 `git pull --ff-only` 同步。维护源码、Windows stage、Web stage 的 99 个 C# 文件逐字节一致；后续验收脚本／交付文档提交不改变游戏产物来源。
- Windows：`release/TouhouWuxiaSurvivor_alpha-0.1.9.exe`，195,251,800 字节，文件版本 0.1.9.0；SHA-256 `12DFE3D520DEC5EA88185FB73D63914ABB69E466872ADCDAE3D2237928E1A867`。内嵌 .NET 8.0.6 与 PCK，排除全局 .NET 的仅 EXE 隔离目录内完成 36 项独立检查。交付为本地 EXE，没有声称已上传 GitHub Release 二进制资产。
- Web 构建 `20260913-051919-061`，启用 `alpha-0.1.9-e65d63d-20260912T214101Z`。同一 C# 游戏项目；Windows Godot 4.7.1／.NET 8，Web 兼容工具链 Godot 4.6.1／.NET 9；仍使用正式优化 Mono、WebGL2 单线程，无共享内存仍可运行，未启用 WebGPU。
- 新内容：单主线有限定式、正常成长暂缓与二次确认、相容技法、持续精进、18 分钟角色首领、有限回收窗口和自愿续战。灵梦／魔理沙保留唯一稳定身份与自机／首领双档案，本局严格排除自机。玩法明细与所有旧日志保存在根 `CHANGELOG.md`，本轮不改视角或重绘美术。
- 核心 68／68、翻译 1108／1108、JavaScript 22／22、字体 1294 字符；Web 新成长 12 项、20 次原始加载、raw／gzip × 有／无隔离四组各五项流程、双角色普通／注入颜色、语言、六类加载场景、小地图 DPR 1／3、23 条图鉴四视口、选人立绘和标题均通过。
- 性能样本：桌面 Edge／RTX 4070 Ti，无隔离、无共享内存，双角色双语的 40／180／320 三档全部平均／最低 60 FPS；最高档 320 敌人、约 1591 弹幕，灵梦核心 P95 1.9／2.0 ms，魔理沙 5.2／5.3 ms。没有减少压力预算或用模拟 ticks 冒充绘制 FPS；这不是物理手机／平板 60 FPS 承诺。
- 平衡与持续性分开记录：四定式无辅助三种子胜局为 2／3、2／3、1／3、2／3，成功局 1097.2–1141.8 秒，首领登场前 26 次成长。同投入 DPS 保持原 0.8–1.2 比率门槛；双角色一小时测试使用明确的恢复辅助，只用于稳定性，不能当胜率证据。
- **公网直连验收完成**：全新浏览器显式 `--no-proxy-server`，桌面 9.473 秒、触屏模拟 11.038 秒、无隔离触屏 9.293 秒进入标题并完成游戏操作；设置保存刷新、多指移动／慢移／闪身和竖屏暂停通过，无页面异常或失败请求。仅代表当次本机线路，不承诺所有地区、时段或实体移动设备同速。
- 核心计量下载 **21,637,091 字节**：PCK gzip 9,777,619，WASM gzip 11,859,472，不含页面／脚本／协议。活动目录全部 gzip 文件合计 21,736,648 字节，不能与核心计量混用。原有减包、BBR、Caddy 路由与无缓存入口／不可变资源路径机制保留；既有根站点、tusharedata、test 路由检查通过。
- 公网入口 SHA-256 `d3609864fbcb9d1552db5fa7641e2fa68f9be152c4ac3c7252f83e1c0cd522d2` 与固定 SSH 主机上的 `/srv/touhou-survivor/active.json` 相符，版本、源码提交和 Windows 哈希一致。`deployment.json` 不作为公共 HTTP 入口发布；不要因两个探索性 404 额外开放元数据。
- 回滚备份 `/srv/touhou-survivor/backups/alpha-0.1.9-e65d63d-20260912T214101Z/`。旧日志从 alpha-0.1.8 起与发布前逐字节相同；已有 EXE 与失败候选保留。首候选预览恰好停在首星冷却边界，修正仅作用诊断初始状态后重建双端；图鉴旧 22 条断言补为 23 条及两角色身份验证，未降低原有性能／颜色／兼容门槛。
- 证据：`artifacts/growth-release-20260913/`、`artifacts/alpha-0.1.9-export-validation/report.json`、本 Web 构建报告及活动部署目录的 `public-verification/report.json`；独立交付日志／回执为 `release/CHANGELOG_alpha-0.1.9.md`、`release/RECEIPT_alpha-0.1.9.json` 和 `artifacts/release-receipts/alpha-0.1.9.json`。已打开的旧网页需要刷新或重新打开以读取新入口。
