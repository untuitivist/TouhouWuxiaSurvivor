...
- Closer orbit input alone failed and is preserved, so it is not treated as a balance fix. Further tracing finds a gameplay integration gap: new Boss stars bypass old Projectile-only orb/beam/signature clears. This undermines the existing defensive techniques (and makes Boss Reimu fail to clear player stars).
- Fixing the shared interaction, not inflating raw damage: segment clear targets the chosen faction across both dense stores with one original budget and visual-radius geometry; beam/signature also include hostile stars. Dead stars are marked and gravity invalidated, never removed during active span traversal. New mixed-store tests cover budget sharing, friendly preservation, Boss targeting, signature and beam clears.
- Native build has zero warnings/errors; all 70 core cases and equal-investment DPS gates pass. First unassisted 18-run sample exposes Reimu auxiliary-focused 0/3 and balanced 1/3 versus ranged-focused 2/3; all Marisa policies have wins. Do not waive the failed auxiliary gate.
- The shared diagnostic pilot always circles a Boss at radius 320, even after investing mainly in 85–115-radius orbs. It now chooses a close orbit only when the actual learned orb investment supports that playstyle; this changes test inputs, not player auto-movement, Boss HP, damage or invulnerability. Retain the failed sample and retest.
- Core correction is implemented: no MainlineStance type, sibling exclusion, fourth-choice gate, confirmation state, charged ofuda or birth-time star stance remains. Each ability has independent meaningful training; auxiliary damage reaches actual field/orb/beam systems and medicine gains exactly one integer HP per training. Offers expose distinct character abilities without mandating the initial weapon.
- Initial build caught one stale stance variable in the test logger (fixed). Next core run passes all new freedom/combination/budget/live-effect tests and both one-hour assisted soaks; 69/70 overall because an old Marisa offer assertion still mandates stars in every hand. Replaced that obsolete requirement with distinct legal directions; no gameplay or performance threshold waived.
- Tool-only errors preserved: one read used presentation instead of core for GameText.Catalog.cs, corrected immediately; one patch targeted a substring rather than a full PowerShell line, so no edit applied until the correct full-line hunk.
...
- High-load desktop Edge on RTX 4070 Ti: both languages and heroes average/minimum 60 FPS across the sampled 40/180/320 scenarios. At 320 enemies / about 1591 projectiles, core P95 is 1.9–2.0ms for Reimu and 5.2–5.3ms for Marisa. This is neither physical phone performance nor a promise of unlimited entities or star capacity.
- Balance evidence: equal-investment single-target DPS stays within the existing 0.8–1.2 ratio at 0/4/8/12 points. Four routes' three-seed unaided wins are 2/3, 2/3, 1/3, 2/3; successful times 1097.2–1141.8s and 26 pre-Boss choices. Distinguish these limited pilot results from the 60-minute recovery-assisted stability tests and human playtesting.
- Final journal diagnosis: replacing the generic Boss with two character profiles increases entries from 22 to 23 and enemy-category cards from five to six. The verification now asserts expected category totals, unique pagination and the exact four ordinary enemy/two character Boss identities instead of retaining an obsolete last-page count. The test-only change follows the frozen product commit.
- Public source identity comes from pinned SSH /srv/touhou-survivor/active.json plus the direct HTTP entry checksum; deployment.json is not exposed as a root or immutable-release URL. Do not add a public metadata route merely to accommodate an incorrect verification URL. Core gzip transfer is 21,637,091 bytes; all compressed site files total 21,736,648 bytes, so these are different metrics.
- Initial implementation audit (historical; statements below describe the pre-change baseline):
...
- Current core owns one player BuildState and one Marisa state; Enemy has no character identity and the Boss is a generic ring/fan pattern. Stars have no ownership and their gravity loop only considers stars and enemies. Dual-role encounters therefore need explicit instance/attack ownership and a bounded Boss ability state, not just a different sprite.
- Existing Windows exporter preserves old EXEs and requires committed game inputs; deployment requires exact clean-source Windows/Web artifacts, four Web compatibility reports, batch/color checks, bilingual load gates and language verification. Keep these guards intact. Naming is TouhouWuxiaSurvivor_alpha-X.Y.Z.exe; latest local artifact is alpha-0.1.8, remote activation still needs direct verification.
- Tool environment does not put dotnet on PATH; the installed SDK is C:/Program Files/dotnet/dotnet.exe. Native Windows OpenSSH exits 255 even for -V with no stderr; bundled Git OpenSSH starts successfully, so use its strict-host-key path for remote checks. The authorized private key is resolved locally and will never be read into logs or uploaded. Literal code searches use rg -F to avoid escaped-parenthesis parsing errors.
- Boss death cleanup must mark owned projectiles/stars dead during damage iteration and compact only after the active span finishes; removing components inside the damage callback would invalidate live iteration and reintroduce entity/render corruption.
...
- BuildState.CanChoose checks hero, minimum level, required ability rank and maximum rank; behavior traits are accumulated by bitwise OR. There is no branch-choice identity or sibling-exclusion rule. UpgradeOffers prioritizes an unlock and a behavior/training offer, encouraging lateral acquisition.
- RunState.BossArrival is 240 seconds; defeating the Boss immediately sets Won. This is a four-minute arrival, not a guaranteed five-minute run. Purification takes seven seconds of local progress and grants an extra choice without a player-level increase.
- Reimu starts with Ofuda and Marisa with Stars. Existing Reimu homing/explosion, field selection/binding and orb clearing/launch are deliberately compatible. A new major fork must not silently turn those pairs into exclusive choices.
- Marisa already repeats mass median, distribution spread and new-star lifetime training. Median/sample logarithms cap at 60; damage radius caps at 90; storage caps at 96 stars. Longer training is not evidence of unlimited effective growth or safe performance. At baseline 0.18-second emission, 96 stars correspond to about 17.28 seconds of lifetime before saturation, ignoring other removals; faster casting reaches that pressure earlier.
- Design implication, not a measured fun result: longer time and deeper prerequisites alone cannot preserve exclusive builds if every upgrade remains legal forever. Propose one mutually exclusive casting/age-profile choice per hero, retain compatible effects, and deepen only the selected mainline.
- Draft docs/run_growth_design.md separates confirmed intent from proposed 18-minute Boss arrival / 18–22-minute standard run, 20–26 total choices and a fourth-choice fork. Reimu contrasts casting cadence; Marisa contrasts young/mature star tearing without restricting independent mass/lifetime training or changing many-body gravity. These are new working designs, not canon spell claims or implemented content.
- Companion abilities remain unlockable and mutually compatible with bounded refinement, not another infinite tree or a new slot system. A branch identity stays exclusive through optional post-victory continuation; existing non-conflicting effects stay combinable. New-star-only timing, saturated upgrade feedback, capacity tradeoffs, equal-investment DPS and performance acceptance are explicit.
- Documentation review: both sides are directly reachable at the proposed offer; only an explicit selection closes the sibling, and refinement/continuation cannot reopen it. This is a consistency review of proposed rules, not an executed gameplay test. Six changed Markdown files decode as UTF-8 without BOM; all ten design links and both new entry links resolve, and the diff contains no runtime/asset/version/changelog changes.

# Direct Download Investigation — 2026-09-11

- Final state: the derived payload is live, BBR remains active after persistence and repeated tests, Caddy PID 1001 has not restarted, both config hashes and unrelated routes are unchanged. Three full public direct browser flows pass at startup 17.751/26.171/31.445s; physical phones/tablets and reboot persistence are not empirically tested.
- Final validation: Python 20/20, JavaScript 22/22, 1275-glyph coverage; six loading scenarios, twenty raw reads, four complete compatibility groups, language persistence and both heroes at all three existing load gates pass. Highest load for both heroes has mean/minimum 60 FPS on the desktop GPU.
- Persisted the measured BBR configuration in two dedicated files after A/B/A validation. The temporary timer had already expired and unloaded before installation; attempted stops report that fact, then only the dedicated sysctl file is explicitly applied. Caddy hashes/qdisc stay identical. A fresh second direct cold browser run passes in 25.290s after persistence.
- Font preflight caught four newly introduced changelog glyphs (硅/站/谷/针); reword the maintenance log using existing covered terms instead of changing game/font assets. Public verifier is hardened to explicitly disable inherited browser proxies and label its direct network mode.
- Further root-cause evidence: the active direct CUBIC socket had cwnd=1 and 562,727 retransmitted bytes / 4,046,033 sent bytes. Kernel 6.8.0-124 provides tcp_bbr; pinned Linux v6.8 tcp_minisocks.c confirms newly accepted sockets select the current default unless explicitly locked. No listener restart is required.
- Ran a runtime-only BBR trial with a 240-second automatic revert-to-cubic timer. Same no-proxy 1 MiB HTTP/1.1 range from the same original URL completed in 3.548/3.615s at 295,556/290,058 B/s; payload prefixes match the original byte-for-byte. Reverting to CUBIC reproduced the problem: 638,976 bytes in 35.007s, 18,252 B/s, timed out again.
- Complete cold browser test with BBR passed in 20.549s, HTTP/2, direct remote IP 170.106.119.27, both exact gzip bodies received (21,610,672 total), no failed requests or page errors; actual title screen screenshot inspected. The older CUBIC browser baseline was deliberately stopped after sampling, not mislabeled a 900-second timeout.
- This transport setting affects new TCP connections across the server, not only a URL path. No Caddy restart, DNS, route, qdisc, paid service or replacement kernel is involved. Durable config is tracked separately and will be installed only after this A/B/A check; retain rollback instructions and verify again afterwards.
...
- First new balance pass: 53/53 core and 997/997 localization passed, native build clean; equal-point routes have bounded distinct output. Unassisted pilot could survive but failed to finish Marisa boss fights by 420s: nearest-target acquisition spent too much damage on fodder. Added finite-range mass-weighted attraction (boss/elite priority) rather than an across-the-board damage buff; rerun required.
- User clarified original assets take priority unless specially requested. The previously referenced other/ directory no longer exists; the supplied raw pack is now alongside the Git root under 东方原作弹幕游戏素材包/东方Project游戏解包内容. Found the exact original Marisa mushroom in TH17.5 data/sprite/marisa/mushroom000.gif (also a player variant exists). Inspected both; selected the original 000 sprite, converted with Aseprite without redrawing, and retained source/output hashes. The prior self-drawn PNG was moved within the verified workspace to the art source folder, not deleted or shipped.
...
- User asks for independent random star masses and a new release; the earlier shared-budget interpretation was explicitly rejected. One shared C# Windows/Web project, existing art/controls and history remain unchanged. Prior tests do not validate this new design.
- Confirmed design: sustained proximity tearing; enemy-attracted stars grow into enemy-attracting planets; damage, mass/distribution and lifetime are separate growth axes. Generation-time mass randomization stays stable, with a guaranteed core for planet growth. Healing uses mushrooms/herbs; beam widens and later tracks the nearest enemy, not player movement.
...
- The standalone stream-only probe reproduces the cancellation without Godot or the game loader. Browser-consumed bytes and SHA-256 are identical to the pack on disk, including the failing attempt. Direct native arrayBuffer comparison passed 16 attempts. No AbortSignal event was observed. Response/reader retention, chunk copies and a manually pulled measuring stream were falsified as reliable remedies; speculative loader modifications were reverted. An upstream cause is not established; raw-stream Web acceptance remains open rather than suppressing request errors.
- Final audit: 29 core tests and desktop UI/profile/settings/render-cache smoke pass; five real-render fixtures plus F3 inspected. Original sources and outputs match their SHA manifests, historical published changelog entries remain byte-identical, and the alpha-0.0.9 EXE retains SHA-256 `e21bfedec06b3dd1f2e730a97776c5b14bf609f7361fc73d4854509c42043ae8`. No release, push or deployment was performed.
- Accepted equal-size OpenGL fixture (RTX 4070 Ti, 1280x720, 320/1600/400 entities, one-second warmup then three-second sample): original 6971 draw calls / 58.67 ms frame interval; batched 806 / 5.03 ms. All five real-render captures have no drawing/shader errors. Original-art crops, sprite visibility, layering and icon were visually inspected. Device/render backend results are not transferable phone FPS guarantees.
- Web high-density fixture passed desktop Edge touch/DPR 1 and 3: both report a 1280x720 render target, 817 draw calls and 60 FPS in this environment. Full gameplay regression is still required; no physical phone/tablet test has been performed.
- Six seeded full-run balance reports from isolated pre-change .NET sources exactly match the changed runtime (time, kills, level, seals, HP and grazes). Added storage/order, hit-overflow and allocation-free query tests; terminal-phase render invalidation is covered so caching cannot freeze a stale final HUD.

- Baseline real OpenGL dense render fixture initially measured 6971 draw calls. The first batched capture exposed HUD DrawStyleBox calls still targeting the parent outside its draw callback; corrected those to the active retained surface and do not count that errored capture as an accepted benchmark. Use an elapsed-time warmup/sample window rather than a short frame count, since the optimized renderer can complete 180 frames before engine timing monitors refresh.
- Windows rejected a combined generated patch above its command-line length limit; applied the same renderer transformation as one bounded apply_patch per file. No partial code edit occurred on the rejected launch.

- Dense component stores and separate enemy/projectile/pickup systems now replace high-frequency List-based updates. Enemy components remain managed data to preserve stable encounter identity; projectile/pickup components are contiguous value data. Collision queries use value enumerators, homing identities are indexed, and ordinary hit histories use four inline slots with overflow only when required. Stable compaction preserves processing order.
- Original deterministic core suite remains 26/26. The same 1200-tick dense fixture yields identical final ticks/HP/projectile/graze/spell counts. Local desktop .NET baseline 320-enemy/1600-projectile mean 0.690 ms and 122508 B/tick becomes 0.364 ms and 158 B/tick. These are simulation-only desktop measurements, not phone FPS. The initial sparse baseline froze at choices; a corrected isolated HEAD baseline resolves choices and records all 1200 ticks.
- Inspected original TH10 bullet/etama.png and etama2.png directly: they contain red/violet pellets, ofuda, stars, orbs and item sprites suitable for traceable crops. Prefer these over procedural projectile geometry.

- Confirmed active csproj explicitly compiles only game/**/*.cs; src ECS is excluded. Active enemy/projectile/pickup data are mutable reference objects, and each projectile eagerly constructs a HashSet even when it is a hostile single-hit bullet. Homing also scans Enemies.Find per projectile; spatial grid queries allocate iterator objects.
- WorldDrawing submits 570 terrain tiles plus procedural grass/flowers/trees each frame; GameCanvas redraws even paused. Project stretch is canvas_items at native display pixels, so high-DPI phones can rasterize far more than the 1280x720 logical frame. These are concrete optimization targets, not yet proven percentages of mobile frame time.
- Original source pack is available adjacent to the repository under 东方原作弹幕游戏素材包/东方Project游戏解包内容. Existing normalization manifests already cite TH08/TH10/TH19 player/enemy assets. Earlier guessed BattlefieldDrawing.cs and packs/base.json paths do not exist; use WorldDrawing.cs and build_manifest.json instead.

- Active gameplay lives under game/core and game/presentation, while a legacy src tree is still present. GameRoot uses a plain RunState with batched Step rather than one Node per projectile, but the presence of legacy ECS documents is not proof that the rebuilt active runtime is ECS.
- CombatDrawing currently emits multiple immediate-mode circles/lines/polygons per projectile and LINQ filters per frame. The renderer needs measurement and batching review in addition to simulation storage changes. No mobile-device performance claim is supported yet.
...
## Full WebGPU Validation Findings — 2026-09-09 UTC

- The actual maintained C# game exports with the integrated threadless template and unchanged shared source/assets. Runtime-only bootstrap selects mobile/webgpu; the default compatible build pointer is preserved. Actual title, Reimu combat and Marisa combat all start on a hardware WebGPU game canvas with no captured GPU/runtime errors. This advances beyond the JS-only lab; full regression and loss/unavailable tests still remain.
- Workstation has 28 logical CPUs and about 80 GiB RAM; Ubuntu-22.04 is stopped. ADB is not on PATH, so physical-mobile access is not established. Existing Web builds already use unique dated staging directories; no live deployment is needed.
- GitHub comparison using fork tag names is not a reliable integration diff: candidate 4.6.2-stable comparison returned 404, and the Mono fork tag resolves to its custom revision (zero diff). Resolve upstream commit identities before generating a patch.
- Fixed candidate and Mono build sources are reachable via direct HTTPS; Web search again returned no usable output. Existing isolated threadless toolchain cache includes engine, Emscripten, .NET, generated glue and export templates, so investigate reuse without altering it. Candidate additionally compiles SPIRV-Tools and vendored Tint; browser support alone does not meet this native build gate.
- Baseline is clean 78bda02, one local commit ahead of origin/main. The prior lab is rendering-only; current game/runtime/source is still alpha-0.1.7. Full validation must first solve or reproduce the real Mono + WebGPU engine gate.

---
# Actual-Game Full Validation Findings — 2026-09-09 UTC

- CRITICAL: browser-compositor comparisons of six actual unchanged static game fixtures all fail. In the 2,320-instance fixture WebGPU displays UI, the hero, some scenery and enemy outlines but omits the dense sprite population and ground seen in WebGL2. Captured C# counts remain identical. The 12 captures are nonblank and use verified hardware backends; mismatch is 66.59–71.54%, not a blank-frame or tiny antialiasing difference. WebGPU numeric FPS gates therefore do NOT establish equivalent rendered work or a speedup. These measurements are retained only as diagnostic timings and fail overall visual/performance acceptance.
- Visual comparison artifacts: artifacts/webgpu-validation/2026-09-09T21-24-20-589Z/visual-comparison. Manually viewed both performance images; the visible omission is unambiguous. The earlier title-only smoke was insufficient.
- WebGPU batch readback fails before color verification with a null Image; the pinned driver texture_get_data implementation explicitly returns an empty vector on the first asynchronous readback. This is an engine/test-contract incompatibility, not proof that the visible game sprites are black. The old injected vertex-color stress is WebGL-specific and cannot establish WebGPU color-state coverage.
- WebGPU raw UI checks complete both hero journeys, touch and upgrades, but the desktop reload check intermittently restores MasterVolume=1 instead of 0.36. Both gzip deployment variants stall after touch; task-owned verifier trees are ended after 11+ and 7+ minutes without progress, with interruption records retained. Diagnostic evaluate/screenshot/teardown now have bounds; no assertion was relaxed.
- Manually reviewed both backend title captures: same layout, artwork and colors, with expected animation-frame differences. This does not replace deterministic fixture pixel comparisons or the failed engine readback gate.
...
- Reference traversal alone is insufficient: 37.86 mean FPS/minimum 8, P95 core 16.3 ms. Saturated CDP sampling attributes about 4.97 of 6 seconds to one WASM interpreter function, not GPU drawing. Applying supported Mono interpreter inlining hints to arithmetic/collision helpers retains shared source and avoids a new runtime/toolchain. Inlining hints alone do not resolve the failure (38.49 FPS/minimum 7).
- Root cause confirmed against pinned source: Godot b94985982 always calls mono_wasm_load_runtime(1), including release. Official .NET 9.0.19 src/mono/browser/runtime/runtime.c explicitly sets interp_opts="-all" whenever debug_level is nonzero. Its debug_level=0 path enables interpreter optimizations. The saturated symbol 3247 resolves to mono_interp_exec_method.
- Correct the pinned template source to debug level 1 only under DEBUG_ENABLED and level 0 in release. Build into a new immutable output directory, retain the previous template/feed, and require matching optimized-runtime metadata and hash for subsequent exports. Shared game rules and browser isolation behavior stay unchanged.
- Optimized runtime passes correct-mass high load at 60 FPS; final 7aca71e dual-platform export and complete compatibility/UI/color/performance gates pass. Source pushed and activated as alpha-0.1.8-7aca71e-20260911T133928Z.
- Direct public download remains slow: 180-second attempt stops near 4.02/32.54 MB. Separate existing-proxy browser checks pass all three entrances; direct header and protected-route checks pass. Both reports are retained. Full delivery receipt and limits are in docs/deployment.md; no physical mobile claim.
## Growth Release Verification — 2026-09-13

- Final visual corrections verified in artifacts/render-performance/growth-candidate-20260913-050646: dark readable build branches, separate training row and visible original Boss sprites with rainbow beam. Both native batch suites pass again. Native core 68/68, localization 1108/1108 and platform JavaScript 22/22. Historical alpha-0.1.8-and-earlier changelog text remains exact; changed text decodes as UTF-8 without BOM.
- Default unassisted seed 42 wins for both heroes, preserving the existing full-Web journey fixture rather than weakening its success criterion. Four stances use the same candidate-only pilot; Rapid/Charged/Young/Mature win 2/2/1/2 of three seeds. Failures retained; no claim that every build or human run is balanced equally.

- Full native presentation builds with zero errors/warnings; 54 added English entries bring localization to 1102/1102. Existing font still needs regeneration and rendered validation.
- Equal-DPS fixture must disable new independent Boss ability state, not just the legacy Timer. Also found the new arbitrary 24-unit Boss hitbox excluded half of baseline straight ofuda at 300 units; restored the established 32-unit combat radius rather than changing damage or relaxing the 0.8–1.2 matched-DPS gate.
- Heavy-star test now compares sampled mean with the actual lognormal expectation after extended training, retains rare Boss-mass samples and no-quota invariants; it no longer assumes the superseded unlimited 1.32^rank growth curve.
- Initial new test compile hit C# top-level Check name resolution despite using-static; fixed by an explicit class-local forwarding method. All logs retained. Local date independently verified as 2026-09-13 in Asia/Shanghai.
