...
public partial class GameRoot
{
    ...
    private void ShowJournal()
    {
        ...
        var panel = Modal("journal", "FIELD JOURNAL  /  夜境图鉴", "见其形，知其所长。", 1160, 660);
        for (var index = 0; index <= Enum.GetValues<JournalCategory>().Length; index++)
        {
            ...
            var label = category.HasValue ? JournalCatalog.CategoryName(category.Value) : "全部";
        ...
        for (var index = 0; index < visible.Length; index++)
        {
            ...
            card.TooltipText = entry.Name + " · 点击查看详情";
        ...
        var back = ui.Button(panel, "返回", new(32, 556, 212, 66), NavigateBack);
        var previous = ui.Button(panel, "上一页", new(474, 556, 176, 66), () => { journalPage--; journalSelected = ""; ShowJournal(); });
        ...
        var next = ui.Button(panel, "下一页", new(860, 556, 268, 66), () => { journalPage++; journalSelected = ""; ShowJournal(); });
        next.Disabled = journalPage == pages - 1;
        ui.Label(panel, $"已实现 {entries.Length} 项 · 点击卡片查看", new(252, 565, 209, 53), 16, Palette.Muted);
    ...
    private void ShowJournalDetail(JournalEntry entry)
    {
        ...
        ui.Label(panel, "局内同源素材 · 静态展示", new(32, 420, 288, 30), 17, Palette.Gold);
        ...
        detail.Text = entry.Summary + "\n\n" + entry.Details + "\n\n设定与改编\n" + entry.Source +
            "\n\n素材路径\n" + JournalCatalog.ArtRoot + entry.Asset +
            (entry.Category == JournalCategory.Training ? "\n\n修习类共用阵纹作分类图标，不表示新增战斗特效。" : "") +
            "\n\n素材仅用于内部原型验证；来源记录不等于公开发行授权。";
        ui.Button(panel, "向上阅读", new(32, 466, 138, 66), () => detail.GetVScrollBar().Value -= detail.Size.Y * 0.7);
        ui.Button(panel, "向下阅读", new(182, 466, 138, 66), () => detail.GetVScrollBar().Value += detail.Size.Y * 0.7);
        ui.Button(panel, "返回卡片", new(32, 556, 288, 66), ShowJournal, true).GrabFocus();
        ui.Label(panel, "滚动阅读全部详情 · 返回保留分类与页码 · Esc 返回", new(350, 570, 778, 44), 19, Palette.Muted);
    }
