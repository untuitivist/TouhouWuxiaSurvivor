...
public partial class GameCanvas
{
    ...
    private void DrawReimuField()
    {
        if (Run?.Field is { } field)
        {
            ...
            OriginalEffect("reimu_seal_ink", center, bounds.Size, Palette.Alpha(Colors.White, 0.52f * fieldOpacity));
            OriginalEffect("reimu_aura", center, bounds.Size * 0.9f, Palette.Alpha(new Color("ffb4c4"), 0.16f * fieldOpacity));
            ...
            {
                OriginalEffect("reimu_talisman", corner, Vector2.One * 26, Palette.Alpha(Colors.White, fieldOpacity));
            }
        }
    }
    ...
    private void DrawMarisaBeam()
    {
        ...
        if (beam.Warmup > 0)
        {
            ...
            OriginalEffect("marisa_cast", origin, Vector2.One * (32 + charge * 36), Palette.Alpha(Colors.White, 0.35f + charge * 0.5f), ReducedMotion ? 0 : -Clock);
            return;
        }
    ...
    private void Star(Vector2 position, float radius, Color color, float rotation)
    {
        ...
        OriginalEffect("star", position, Vector2.One * radius * 2, color, rotation);
    }
}
