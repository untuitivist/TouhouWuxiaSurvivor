...
public partial class GameCanvas
{
    ...
    private void DrawHeroFields()
    {
        if (Run?.Field is { } field)
        {
            ...
            surface.DrawRect(bounds, Palette.Alpha(Palette.Red, 0.055f * fieldOpacity));
            surface.DrawRect(bounds, Palette.Alpha(Palette.Red, 0.7f * fieldOpacity), false, 2);
            surface.DrawRect(bounds.Grow(-12), Palette.Alpha(Palette.Paper, 0.22f * fieldOpacity), false, 1);
            surface.DrawArc(center, field.HalfSize * 0.8f, 0, MathF.Tau, 64, Palette.Alpha(Palette.Red, 0.16f * fieldOpacity), 1);
            ...
            {
                surface.DrawRect(new(corner - new Vector2(6, 10), new(12, 20)), Palette.Alpha(Palette.Paper, fieldOpacity));
                surface.DrawLine(corner - Vector2.Down * 6, corner + Vector2.Down * 6, Palette.Red, 3);
            }
        }
        ...
        surface.DrawColoredPolygon([origin + side, target + side, target - side, origin - side], Palette.Alpha(Palette.Violet, 0.26f * opacity));
        surface.DrawLine(origin, target, Palette.Alpha(Palette.Gold, 0.28f * opacity), beam.HalfWidth * 1.45f);
        surface.DrawLine(origin, target, Palette.Alpha(Palette.Paper, 0.6f * opacity), beam.HalfWidth * 0.65f);
        surface.DrawLine(origin + side, target + side, Palette.Alpha(Palette.Jade, 0.7f * opacity), 2);
        surface.DrawLine(origin - side, target - side, Palette.Alpha(Palette.Violet, 0.7f * opacity), 2);
