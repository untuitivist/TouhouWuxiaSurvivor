...
public partial class GameRoot
{
    ...
    {
        AddVideoOption(panel, GameText.Get("触控按钮"), "touch_mode", 209, [GameText.Get("自动检测"), GameText.Get("始终显示"), GameText.Get("隐藏")], profile.Data.TouchMode, selected =>
        {
            profile.Data.TouchMode = selected;
            SaveSettings(false);
        });
        ui.Label(panel, GameText.Get("左侧摇杆移动，右侧按住慢移、点按闪身。\n支持同时移动、慢移和闪身；抬手、暂停或切到后台会清除触控。\n右上角可暂停或查看构筑，升级时直接点击卡片。\n手机请横屏游玩；键盘设置与触控相互独立。"), new(36, 285, 988, 145), 21);
        ui.Button(panel, GameText.Get("切换诊断信息（F3）"), new(36, 453, 480, 52), ToggleDebug);
        ui.Label(panel, GameText.Get("诊断只读，不暂停，也不会改变战况。"), new(544, 463, 480, 43), 17, Palette.Muted);
    }

    private void BuildWebVideoSettings(Control panel)
    {
        AddVideoOption(panel, GameText.Get("帧率上限"), "fps_limit", 209,
        ...
        ui.Button(panel, GameText.Format($"减少震屏：{(profile.Data.ReducedMotion ? GameText.Get("开") : GameText.Get("关"))}"), new(36, 285, 480, 54), () =>
        {
            profile.Data.ReducedMotion = !profile.Data.ReducedMotion;
            SaveSettings();
        });
        ui.Button(panel, GameText.Get("切换浏览器全屏"), new(544, 285, 480, 54), ToggleWebFullscreen);
        ui.Label(panel, GameText.Get("网页画面随浏览器大小适配。\n窗口分辨率、无边框窗口和垂直同步由浏览器管理，\n这里不显示无效的桌面设置。\n帧率与减少震屏即时保存；全屏是否可用取决于浏览器。"), new(36, 374, 988, 146), 21, Palette.Muted);
    }
