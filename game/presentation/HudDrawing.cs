...
public partial class GameCanvas
{
    ...
    private void DrawHud()
    {
        ...
        FittedText(Run.Hero == HeroKind.Reimu ? GameText.Get("博丽灵梦") : GameText.Get("雾雨魔理沙"), new(28, 32), 138, 22, Palette.Paper);
        Text(GameText.Format($"修习 {Run.Level}"), new(178, 31), 16, Palette.Gold);
        ...
        Text(Run.Build.SignatureUnlocked ? GameText.Get("擦弹蓄势 · 满槽自动清弹") : GameText.Get(Run.Hero == HeroKind.Reimu ? "梦想封印 · 尚未解锁" : "满蓄势魔炮 · 尚未解锁"), new(989, 77), 14, Palette.Muted);
        Text(GameText.Get("历 练"), new(28, 679), 13, Palette.Jade);
        ...
        FittedText(GameText.Format($"{GameControls.Hint(GameControls.Dash)} 闪身 · {(Run.DashCooldown <= 0 ? GameText.Get("就绪") : $"{Run.DashCooldown:0.0}s")}"), new(654, 683), 173, 16, Run.DashCooldown <= 0 ? Palette.Paper : Palette.Muted);
        Bar(new(654, 697, 173, 3), 1 - Run.DashCooldown / Run.DashInterval, Palette.Gold);
        FittedText(GameText.Format($"{GameControls.Hint(GameControls.Focus)} 慢移   Esc 暂停"), new(1032, 690), 218, 14, Palette.Muted);
        DrawMinimap();
        DrawObjective();
        var boss = Run.Boss;
        if (boss != null)
        ...
        if (Run.Time < 10)
        {
            ...
            CenterText(Run.Hero == HeroKind.Reimu ? GameText.Get("基础御札起步，修习解锁阵与玉。") : GameText.Get("基础星光起步，修习解锁星尘与魔炮。"), new(640, 578), 20, Palette.Paper, TitleFont);
    ...
    private void DrawMinimap()
    {
        ...
        FittedText(GameText.Get("博 丽 夜 境"), bounds.Position + new Vector2(8, 131), bounds.Size.X - 16, 12, Palette.Muted);
    }
