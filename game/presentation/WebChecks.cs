...
public partial class GameRoot
{
    ...
    private void UpdateWebChecks(double delta)
    {
        ...
        var state = new WebCheckState
        {
            ...
            AvailableUpgradeIds = run == null ? [] : UpgradeCatalog.All.Where(upgrade => run.Build.CanChoose(upgrade, run.Level)).Select(upgrade => upgrade.Id).ToArray(),
            ChoiceAbilities = run?.Choices.Select(upgrade => upgrade.Ability.ToString()).ToArray() ?? [],
            AbilityInvestments = run == null ? [] : ArtCatalog.All.Select(art => run.Build.Investment(art.Id)).ToArray(),
            ...
            BossBullets = run?.Projectiles.Count(projectile => projectile.Hostile && projectile.OwnerId > 0 && projectile.Life > 0) ?? 0,
            BossHomingBullets = run?.Projectiles.Count(projectile => projectile.Hostile && projectile.OwnerId > 0 && projectile.TurnRate > 0) ?? 0,
            BossBeamCount = run?.Boss?.Abilities?.Beam?.Count ?? 0,
            BossBeamActive = run?.Boss?.Abilities?.Beam is { Warmup: <= 0, Remaining: > 0 },
            BossVolleyWarning = run?.Boss?.Abilities?.Volley is { Warmup: > 0 },
