...
public enum UpgradeKind { Unlock, Refine, Behavior, Legacy, Recovery, Training }
...
public sealed class BuildState
{
    ...
    private readonly int[] abilityInvestments = new int[ArtCatalog.All.Length];
    public int Investment(ArtKind ability) => abilityInvestments[(int)ability];
    ...
    public bool TryApply(UpgradeDefinition upgrade, int level)
    {
        ...
        if (upgrade.Kind == UpgradeKind.Training)
        {
        ...
        if (upgrade.Owner == Hero) abilityInvestments[(int)upgrade.Ability] = Math.Min(int.MaxValue - 1, abilityInvestments[(int)upgrade.Ability]) + 1;
