...
public static class ArtCatalog
{
    ...
    [
        ...
        new(ArtKind.Herbs, "药菇调合", "魔理沙 · 调息", "定期在身边留下药菇，拾取回血；满血时保留，有存续时间与数量上限。", "每份恢复 24 点生命；留药与缓释需分别领悟。", "8cdcc8", 5, HeroKind.Marisa, "本作玩法改编：蘑菇与草药调合，不改写原作职业", "药"),
        ...
        new(ArtKind.Power, "威力修习", "修习 · 威力", "威力系数增加基础值的 30%；符卡同样受益。", "", "e6c786", 3),
        new(ArtKind.Haste, "施法精进", "修习 · 节奏", "施放频率系数 +0.22；不加快星群或魔炮伤害脉冲，也不加快药菇生成。", "", "8cdcc8", 3),
    ...
    public static string UpgradeText(ArtKind kind, int rank)
    {
        ...
        var previous = rank == 0 ? default : AbilityTuning.Get(kind, rank);
        var width = rank == 0 ? 0 : AbilityTuning.BeamHalfWidth(rank) * 2;
        return kind switch
        {
            ArtKind.Ofuda => $"每轮御札 {previous.Count} → {next.Count} 张；单札基础伤害 {previous.Damage:0} → {next.Damage:0}。",
            ArtKind.YinYang => $"阴阳玉 {previous.Count} → {next.Count} 枚；基础伤害 {previous.Damage:0} → {next.Damage:0}；回旋半径 {previous.Range:0} → {next.Range:0}。",
            ArtKind.Boundary => $"阵地半边长 {previous.Range:0} → {next.Range:0}；留阵 {previous.Duration:0.0} → {next.Duration:0.0} 秒；每次基础伤害 {previous.Damage:0} → {next.Damage:0}。",
            ArtKind.Stars => $"每单位质量基础撕扯 {previous.Damage:0.00} → {next.Damage:0.00}/秒；继续逐颗向四面散星。",
            ArtKind.Herbs => $"每份药菇恢复 {previous.Damage:0} → {next.Damage:0} 点生命；每 {next.Interval:0} 秒生成一份。",
            _ => $"光束宽 {width:0} → {AbilityTuning.BeamHalfWidth(rank + 1) * 2:0}；持续 {previous.Duration:0.00} → {next.Duration:0.00} 秒；每次基础伤害 {previous.Damage:0} → {next.Damage:0}。"
        };
    }
